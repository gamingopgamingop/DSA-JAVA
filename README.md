...
24. [Matrix Multiplication](#matrix-multiplication)
25. [Sparse Matrix](#sparse-matrix)
26. [Queue Using Array](#queue-using-array)
...
### Output
```
Array after removal: [10, 30, 40, 0]
```
...
### Objective
To multiply two matrices of given dimensions and display the result.

### Algorithm
1.  Read rows and columns for matrix 1 and matrix 2.
2.  Multiply matrices using triple nested loop: `result[i][j] += m1[i][k] * m2[k][j]`.
...
M1: `[[1, 2], [3, 4]]`
M2: `[[5, 6], [7, 8]]`
1.  `res[0][0] = 1*5 + 2*7 = 19`
2.  `res[0][1] = 1*6 + 2*8 = 22`
3.  `res[1][0] = 3*5 + 4*7 = 43`
4.  `res[1][1] = 3*6 + 4*8 = 50`
**Result**: `[[19, 22], [43, 50]]`
...
### Objective
To represent a matrix with many zeros in a space-efficient triplet format (Row, Column, Value).

### Algorithm
1.  Iterate through the dense matrix.
2.  If `arr[i][j] != 0`, store `(i, j, arr[i][j])` in a list.
3.  Print the list of triplets.
...
### Dry Run
**Input**: Dense Matrix: `[[0, 10], [0, 0], [20, 0]]`
1.  `mat[0][1] = 10` -> Triplet: `(0, 1, 10)`
2.  `mat[2][0] = 20` -> Triplet: `(2, 0, 20)`
...
0 1 10
2 0 20
Matrix size: 3x2 = 6 elements
Non-zero elements: 2 (33.3% density)
```
...
### Objective
To implement a Queue data structure using an array, supporting Enqueue and Dequeue operations.

### Algorithm
1.  **Enqueue**: Add element to `rear`. Increment `rear` circularly.
2.  **Dequeue**: Remove element from `front`. Increment `front` circularly.
...
1.  `Insert(10)`: `front=0, rear=0, a[0]=10`.
2.  `Insert(20)`: `rear=1, a[1]=20`.
3.  `Delete()`: `front=1, return 10`.
