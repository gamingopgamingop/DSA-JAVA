...
/**
 * Test class for RemoveElement algorithm validation
 */
public class RemoveElement_Test {
    ...
    public static void main(String[] args) {
        try {
            ...
            writer.println("=== RemoveElement Test Results ===");
            ...
            // Test 4: Multiple removal approaches
            ...
            writer.close();
            System.out.println("RemoveElement tests completed. Results saved to " + TEST_RESULTS_FILE);
    ...
    /**
     * Test basic remove element functionality
     */
    private static void testBasicFunctionality() {
        ...
        // Test removal at index 2
        int index = 2;
        int[] result = removeElement(array, index);
        
        // Expected result: [10, 20, 40, 50, 0]
        int[] expected = {10, 20, 40, 50, 0};
        ...
        writer.printf("  Remove at index %d%n", index);
    ...
    private static void testEdgeCases() {
        ...
        int[] result1 = removeElement(array1, 0);
        int[] expected1 = {30, 40, 50, 0};
        boolean removeBeginningSuccess = Arrays.equals(result1, expected1);
        
        // Test removal at end
        int[] array2 = {10, 20, 30, 40};
        int[] result2 = removeElement(array2, 3);
        int[] expected2 = {10, 20, 30, 0};
        boolean removeEndSuccess = Arrays.equals(result2, expected2);
        
        // Test removal from single element array
        int[] array3 = {20};
        int[] result3 = removeElement(array3, 0);
        int[] expected3 = {0};
        boolean singleElementSuccess = Arrays.equals(result3, expected3);
        
        // Test removal from empty array (should handle gracefully)
        try {
            int[] emptyArray = {};
            int[] result4 = removeElement(emptyArray, 0);
            boolean emptyArraySuccess = result4.length == 0;
            writer.printf("Empty array test: %s (Expected: true)%n", emptyArraySuccess);
        } catch (Exception e) {
            writer.printf("Empty array error: %s%n", e.getMessage());
        }
        
        // Test removal with negative numbers
        int[] array5 = {10, -20, 30, -40, 50};
        int[] result5 = removeElement(array5, 1);
        int[] expected5 = {10, 30, -40, 50, 0};
        boolean negativeNumbersSuccess = Arrays.equals(result5, expected5);
        
        // Test removal with zero values
        int[] array6 = {10, 0, 30, 0, 50};
        int[] result6 = removeElement(array6, 1);
        int[] expected6 = {10, 30, 0, 50, 0};
        boolean zeroElementSuccess = Arrays.equals(result6, expected6);
        
        writer.printf("Edge Case Tests:%n");
        writer.printf("  Remove at beginning: %s (Expected: true)%n", removeBeginningSuccess);
        writer.printf("  Remove at end: %s (Expected: true)%n", removeEndSuccess);
    ...
    private static void testPerformance() {
        ...
        writer.println("Size | Removal (ms) | Shift Operations | Memory Usage (KB)");
        writer.println("------|---------------|----------------|------------------");
        ...
        for (int size : sizes) {
            ...
            int memoryUsage = result.length * 4; // 4 bytes per int
            
            writer.printf("%-8d | %-16d | %-16d | %-18d", 
                size, removalTime / 1000000, shiftCount, memoryUsage / 1024);
        }
        
        writer.println("\nPerformance Analysis:");
        writer.println("• Removal: O(n) - Linear time due to element shifting");
        writer.println("• Shift Operations: n-index-1 elements shifted left");
        writer.println("• Memory Usage: O(n) - In-place modification");
        writer.println("• Best Case: Remove at end (no shifting)");
        writer.println("• Worst Case: Remove at beginning (maximum shifting)");
    }
    
    /**
     * Test multiple removal approaches
     */
    private static void testMultipleApproaches() {
        ...
        int[] manualResult = removeElement(array.clone(), index);
        ...
        int[] copyResult = removeElementArrayCopy(array.clone(), index);
    ...
    private static void testLargeDataset() {
        ...
        long startTime = System.nanoTime();
        int[] result = removeElement(largeArray, index);
        long endTime = System.nanoTime();
        long duration = endTime - startTime;
        
        // Verify removal
        boolean removalSuccess = result[index] == largeArray[index + 1];
        ...
        // Verify last element is set to 0
        boolean lastElementZero = result[result.length - 1] == 0;
        
        writer.printf("Large dataset test: Size: %d%n", largeSize);
        writer.printf("  Removal time: %d ms%n", duration / 1000000);
        writer.printf("  Removal success: %s (Expected: true)%n", removalSuccess);
        writer.printf("  Shift success: %s (Expected: true)%n", shiftSuccess);
        writer.printf("  Last element zero: %s (Expected: true)%n", lastElementZero);
        writer.printf("  Final array size: %d (Expected: %d)%n", result.length, largeSize);
        
        writer.println("\nLarge Dataset Insights:");
        writer.println("• Manual removal scales linearly with array size");
        writer.println("• Performance is predictable: O(n) time complexity");
        writer.println("• Memory usage remains constant");
    ...
    private static void testMemoryEfficiency() {
        ...
        writer.println("Size | Original (KB) | After Removal (KB) | Memory Change (KB)");
        writer.println("------|---------------|---------------------|-------------------");
        ...
        for (int size : sizes) {
            ...
            int[] removedArray = removeElement(originalArray, size / 2);
            int removedMemory = removedArray.length * 4;
            
            int memoryChange = removedMemory - originalMemory;
            
            writer.printf("%-8d | %-15d | %-23d | %-19d", 
                size, originalMemory / 1024, removedMemory / 1024, memoryChange / 1024);
        }
        
        writer.println("\nMemory Analysis:");
        writer.println("• Original memory: O(n) where n is array size");
        writer.println("• After removal: O(n) where n is original size");
        writer.println("• Memory change: 0 KB (in-place modification)");
        writer.println("• Memory efficiency: Excellent (no additional allocation)");
        writer.println("• In-place operations save memory");
    }
    ...
    private static void testAdvancedOperations() {
        ...
        int[] result1 = removeElement(array, 1);
        int[] result2 = removeElement(result1, 2);
        int[] result3 = removeElement(result2, 0);
        
        int[] expectedMultiple = {30, 40, 0, 0, 0, 0};
        boolean multipleSuccess = Arrays.equals(result3, expectedMultiple);
        
        // Test removal with sorted array
        int[] sortedArray = {10, 20, 30, 40, 50};
        int[] sortedResult = removeElement(sortedArray, 2);
        boolean sortedSuccess = isSortedExceptLast(sortedResult);
        
        // Test removal with reverse sorted array
        int[] reverseArray = {50, 40, 30, 20, 10};
        int[] reverseResult = removeElement(reverseArray, 1);
        boolean reverseSuccess = isReverseSortedExceptLast(reverseResult);
        
        // Test removal with duplicate elements
        int[] duplicateArray = {10, 20, 20, 30, 40};
        int[] duplicateResult = removeElement(duplicateArray, 2);
        int[] expectedDuplicate = {10, 20, 30, 40, 0};
        boolean duplicateSuccess = Arrays.equals(duplicateResult, expectedDuplicate);
        
        // Test removal with boundary values
        int[] boundaryArray = {Integer.MAX_VALUE, Integer.MIN_VALUE, 0};
        int[] boundaryResult = removeElement(boundaryArray, 1);
        boolean boundarySuccess = boundaryResult[1] == 0 && boundaryResult[2] == 0;
        
        writer.printf("Advanced operations test:%n");
        writer.printf("  Multiple removals: %s (Expected: true)%n", multipleSuccess);
        writer.printf("  Sorted array: %s (Expected: true)%n", sortedSuccess);
    ...
    private static void testDifferentDataTypes() {
        ...
        int[] positiveResult = removeElement(positiveArray, 1);
        
        // Test with negative integers
        int[] negativeArray = {-10, -20, -30};
        int[] negativeResult = removeElement(negativeArray, 1);
        
        // Test with mixed integers
        int[] mixedArray = {-10, 0, 10, 20};
        int[] mixedResult = removeElement(mixedArray, 2);
        
        // Test with zero
        int[] zeroArray = {0, 0, 0};
        int[] zeroResult = removeElement(zeroArray, 1);
        
        // Test with large numbers
        int[] largeArray = {Integer.MAX_VALUE, Integer.MIN_VALUE, 0};
        int[] largeResult = removeElement(largeArray, 1);
        ...
        boolean typeConsistency = positiveResult.length == 3 && 
                               negativeResult.length == 3 && 
                               mixedResult.length == 4 && 
                               zeroResult.length == 3 && 
                               largeResult.length == 3;
    ...
    private static void testCollectionIntegration() {
        ...
        // Compare with manual removal
        int[] manualArray = {10, 20, 30, 40, 50};
        int[] manualResult = removeElement(manualArray, 2);
        ...
        long manualTime = measureRemoveTime(manualArray);
    ...
    private static void testErrorHandling() {
        ...
        try {
            int[] array = {10, 20, 30};
            int[] result = removeElement(array, -1, 25);
        ...
        try {
            int[] array = {10, 20, 30};
            int[] result = removeElement(array, 5, 25);
        ...
        // Test null array
        try {
            int[] result = removeElement(null, 0, 25);
        ...
        try {
            ...
            int[] result = removeElement(largeArray, 1000000, 25);
        ...
        try {
            ...
            int[] result = removeElement(array, 1, Integer.MAX_VALUE);
    ...
    private static void testBoundaryConditions() {
        ...
        int[] minArray = {10};
        int[] minResult = removeElement(minArray, 0);
        boolean minSuccess = Arrays.equals(minResult, new int[]{0});
        
        // Test maximum valid index
        int[] maxArray = {10, 20, 30};
        int[] maxResult = removeElement(maxArray, 2);
        boolean maxSuccess = Arrays.equals(maxResult, new int[]{10, 20, 0});
        
        // Test index at array length
        try {
            int[] lengthArray = {10, 20, 30};
            int[] lengthResult = removeElement(lengthArray, 3, 40);
        ...
        int[] maxElementResult = removeElement(maxElementArray, 1, Integer.MAX_VALUE);
        boolean maxElementSuccess = maxElementResult[1] == 0;
        
        // Test with Integer.MIN_VALUE as element
        int[] minElementArray = {10, 20};
        int[] minElementResult = removeElement(minElementArray, 1, Integer.MIN_VALUE);
        boolean minElementSuccess = minElementResult[1] == 0;
    ...
    private static void testPerformanceComparison() {
        ...
        for (int size : sizes) {
            ...
            int[] manualResult = removeElement(manualArray, size / 2);
            long manualEnd = System.nanoTime();
            long manualTime = manualEnd - manualStart;
            ...
            // System.arraycopy removal
            long copyStart = System.nanoTime();
            int[] copyArray = generateRandomArray(size, 1, 1000);
            int[] copyResult = removeElementArrayCopy(copyArray, size / 2);
            long copyEnd = System.nanoTime();
            long copyTime = copyEnd - copyStart;
        ...
        writer.println("• Manual shifting is most memory efficient");
    }
    ...
    private static int[] removeElement(int[] array, int index, int defaultValue) {
        if (array == null) {
            throw new IllegalArgumentException("Array cannot be null");
        }
    ...
    private static int[] removeElementArrayCopy(int[] array, int index) {
        if (array == null) {
            throw new IllegalArgumentException("Array cannot be null");
        }
    ...
    private static boolean isSortedExceptLast(int[] array) {
    ...
    private static boolean isReverseSortedExceptLast(int[] array) {
        for (int i = 0; i < array.length - 2; i++) {
            if (array[i] < array[i + 1]) {
                return false;
            }
    ...
    private static long measureRemoveTime(int[] array) {
        long start = System.nanoTime();
        removeElement(array, array.length / 2);
    ...
    private static void generateSummary() {
        writer.println("\n=== Final Summary ===");
        writer.println("RemoveElement Algorithm Testing Completed");
...
/**
 * Performance comparison utility for removal algorithms
 */
class RemoveElementPerformanceComparison {
    public static void compareRemovalApproaches() {
        System.out.println("=== Removal Algorithm Performance Comparison ===");
        ...
        for (int size : sizes) {
            ...
            int[] manualResult = new RemoveElement_Test().removeElement(manualArray, size / 2);
            long manualEnd = System.nanoTime();
            long manualTime = manualEnd - manualStart;
            ...
            int[] copyResult = new RemoveElement_Test().removeElementArrayCopy(copyArray, size / 2);
            long copyEnd = System.nanoTime();
            long copyTime = copyEnd - copyStart;
        ...
        System.out.println("• LinkedList: O(n) - Node traversal to removal point");
        ...
        System.out.println("• Manual shifting is most memory efficient");
    ...
    public static void compareMemoryUsage() {
        ...
        for (int size : sizes) {
            // Manual removal memory
            int[] manualArray = new int[size];
            int[] manualResult = new RemoveElement_Test().removeElement(manualArray, size / 2);
            int manualMemory = manualResult.length * 4;
            
            // ArrayList removal memory
            List<Integer> arrayList = new ArrayList<>();
            for (int i = 0; i < size; i++) {
                arrayList.add(i);
            }
